			...
			<div class="board-contents">
				...
				<c:if test="${sessionScope.userId != null }">
					<div class="write-button">
						<button onclick="location.href='createTipArticle.do'">글쓰기</button>
					</div>
				</c:if>
							...
							<tbody>
								<c:forEach items="${ArticleList }" var="Article">
									<tr>
										...
										<td class="title">
											<div class="text-wrap">
												<a href="selectTipArticle.do?postNo=${Article.postNo }">
													${Article.title} </a>
											</div>
										</td>
	...
	<div class="paigination-wrapper">
		<ul class="pagination">
			<li id="Previous-page"><a
				href="pageNationTipArticleAction.do?arrowDirection=left"
				aria-label="Previous"> <span aria-hidden="true">&#60;</span> <span
					class="sr-only">이전</span>
			</a></li>
			<c:forEach begin="${startPoint }" end="${endPoint }" var="pageNum">
				<li><a href="pageNationTipArticleAction.do?pageNum=${pageNum }">${pageNum }</a></li>
			</c:forEach>
			<li id="next-page"><a
				href="pageNationTipArticleAction.do?arrowDirection=right"
